...
def get_employee_selection_keyboard(employees: List[Tuple[int, str]], show_back: bool = False) -> InlineKeyboardMarkup:
    buttons = []
...
def get_schedule_edit_keyboard(show_back: bool = False) -> InlineKeyboardMarkup:
    keyboard = [
...
def get_date_selection_keyboard(days: int = 14, show_back: bool = False) -> InlineKeyboardMarkup:
    buttons = []
...
def get_slot_selection_keyboard(slots: List, show_address: bool = False, show_back: bool = False) -> InlineKeyboardMarkup:
    buttons = []
    for slot in slots:
...
def get_yes_no_keyboard(action: str, show_back: bool = False) -> InlineKeyboardMarkup:
    keyboard = [
        [
...
def get_employees_count_keyboard(show_back: bool = True) -> InlineKeyboardMarkup:
    keyboard = [
        [
...
def get_worker_management_keyboard(show_back: bool = False) -> InlineKeyboardMarkup:
    keyboard = [
...
def get_period_selection_keyboard(show_back: bool = False) -> InlineKeyboardMarkup:
    """Keyboard for selecting date period"""
    today = datetime.now().date()
    this_month_start = today.replace(day=1)
    if this_month_start.month == 12:
        next_month = this_month_start.replace(year=this_month_start.year + 1, month=1)
    else:
        next_month = this_month_start.replace(month=this_month_start.month + 1)
...
def get_free_time_slots_keyboard(free_time_slots: List[Dict], show_back: bool = False) -> InlineKeyboardMarkup:
    """Keyboard for selecting free time slot to delete"""
    buttons = []
    for slot in free_time_slots:
        ...
        button_text = f"{start_time}-{end_time}"
        buttons.append([InlineKeyboardButton(button_text, callback_data=f"free_time_{slot_id}")])
    if show_back:
        buttons.append([InlineKeyboardButton("◀️ Назад", callback_data="back")])
    return InlineKeyboardMarkup(buttons)
